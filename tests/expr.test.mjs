...
near(evalExpression("2*u + sqrt(cos(u))", { u: 0 }), 1);
...
assert.ok(shortWidth >= 80, "expected minimum width");
assert.ok(shortWidth <= 96, "short expression estimate should stay compact");

const sinLatex = exprToLatex("sin(u)*u+7");
const sinWidth = estimateLatexWidth(sinLatex);
assert.ok(sinWidth >= 120, "expected sin(u)*u+7 to produce a wide block estimate");
